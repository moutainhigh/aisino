...
//	 * @param type
	...
	public SJCaCryptUtil(byte[] corpCert) {
		if ("1".equals(StaticConstant.TAX_CA_TYPE)) {
			...
			this.sStoreName = StaticConstant.CA_TEST_PATH
					+ StaticConstant.SJ_StoreName; // 密钥库名称
...
//			sAlias = (String) jSE.getAliasEnum(kStore).nextElement();
			...
			this.sPin = StaticConstant.SJ_Pin; // 密钥库私钥密码
//			jSE = new javasafeengine();
//			kStore = jSE.getKeyStore("PKCS12", "SUN", this.sStoreName,
//					this.sPin);// 本地证书读取
//			sAlias = (String) jSE.getAliasEnum(kStore).nextElement();
		}
	 ...
	 */
	public byte[] decrypt(byte[] content) throws KeyStoreException,
			IOException, CustomerException {
		if ("1".equals(StaticConstant.TAX_CA_TYPE)) {
	 ...
	 * @throws CustomerException
	 */
	public byte[] decryptLocal(byte[] content) throws KeyStoreException,
			CustomerException {
...
//				this.kStore, this.sAlias, this.sPin); // 获取私钥
//
//		decrypt_data = jSE.envelope(2, encryptData,
//				this.kStore.getCertificate(this.sAlias), oPrivateKey,
//				"RSA/ECB/PKCS1Padding", "SUN", "BC");
//		// 用 私钥解信封
//		boolean isTrue = jSE.verifySign(decrypt_data, signedData, "SHA1/RSA",
//				this.corpCert, "BC");
		// 验签
//		if (isTrue) {
//			return decrypt_data;
...
//		}
	 ...
	 * @throws CustomerException
	 */
	public byte[] decryptMachine(byte[] content) throws KeyStoreException,
			CustomerException {
		String[] con = new String(content).split(",");
		String cryptograph = con[0];
		String signData = con[1];
...
//		decrypt_data = jSE.envelope(2, encryptData,
//				this.kStore.getCertificate(this.sAlias), oPrivateKey,
//				"RSA/ECB/PKCS1Padding", "DatechCrypto", "DatechCrypto",
//				"RsaKey1", "");
//		// 验签
//		boolean isTrue = jSE.verifySign(decrypt_data, signedData, "SHA1/RSA",
//				this.corpCert, "DatechCrypto");
		// ---------------------------------调用加密机结束---------------------------------
...
//		// 用公钥封信封
//		encryptData = jSE.envelope(1, cleartext, cert, null,
//				"RSA/ECB/PKCS1Padding", "SUN", "BC");
//		// 获取私钥
//		PrivateKey oPrivateKey = (PrivateKey) jSE.getPrivateKeyFromStore(
//				this.kStore, this.sAlias, this.sPin);
//
...
//				this.kStore, this.sAlias, this.sPin);
//		// 获取私钥
//		signedData = jSE.sign(cleartext, oPrivateKey, "SHA1/RSA",
//				"DatechCrypto");
//		// ---------------------------------------调用加密机结束---------------------------------------
//
//		ByteArrayOutputStream baos = new ByteArrayOutputStream();
...
//		returnData = baos.toByteArray();
	 ...
	 * 
	 * String cleartext = "航天信息"; String machineNo = "0000016500313026";
	 * QYCaCryptUtil qyutil = new
	 * QYCaCryptUtil(FileUtils.readFileToByteArray(new
	 * File("E:\\skskjca\\ca\\CorpCA\\UserCert.der"))); SJCaCryptUtil sjutil =
	 * new SJCaCryptUtil(FileUtils.readFileToByteArray(new
	 * File("E:\\skskjca\\ca\\qyCA\\UserCert.der"))); byte[] res =
	 * sjutil.encrypt(cleartext.getBytes()); System.out.println("encrypt加密成功:"+
	 * new String(sjutil.encrypt(cleartext.getBytes()))); String strs[] = { "",
