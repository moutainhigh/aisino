 ...
 */
public class ProXml {
	private final static Logger log = LoggerFactory.getLogger(ProXml.class);
	...
	private static String getXml(GlobalInfo globalInfo,
		...
		try {
			...
			Element sjZsbhEle = new Element(XmlPar.SJZSBH);
			sjZsbhEle.setText(data.getSjZsbh());
			eDataDescription.addContent(sjZsbhEle);
			Element qyZsbhEle = new Element(XmlPar.QYZSBH);
			qyZsbhEle.setText(data.getQyZsbh());
			eDataDescription.addContent(qyZsbhEle);
			eData.addContent(eDataDescription);
			Element eContent = new Element(XmlPar.CONTENT);
			if (flag) {
				// 修改String串为空的判断
				String content = encodeData(
						data.getContent(),
						iszip,
						data.getEncryptCode(),
						globalInfo.getPassWord(),
						globalInfo.getUserName()
								+ (StringUtils.isEmpty(data.getQyZsbh()) == true ? "1"
										: data.getQyZsbh()));
	...
	/**
	 * 取得全部明文信息 将传入的xml参数字符串转换成为map by 苏海林
	 * 
	 * @return Map
	 * @author: 张士锋
	 * @throws Exception
			...
			if (node != null && node.size() > 0) {
				for (int i = 0; i < node.size(); i++) {
					...
					if (e1.getName().equals(XmlPar.DATA)) {
						...
						if (e1.getChild(XmlPar.DATADESCRIPTION).getChild(
								XmlPar.SJZSBH) != null) {
							data.setSjZsbh(e1.getChild(XmlPar.DATADESCRIPTION)
									.getChild(XmlPar.SJZSBH).getText() == null ? ""
									: e1.getChild(XmlPar.DATADESCRIPTION)
											.getChild(XmlPar.SJZSBH).getText());
						} else {
							data.setSjZsbh("");
						}
						if (e1.getChild(XmlPar.DATADESCRIPTION).getChild(
								XmlPar.QYZSBH) != null) {
							data.setQyZsbh(e1.getChild(XmlPar.DATADESCRIPTION)
									.getChild(XmlPar.QYZSBH).getText() == null ? ""
									: e1.getChild(XmlPar.DATADESCRIPTION)
											.getChild(XmlPar.QYZSBH).getText());
						} else {
							data.setQyZsbh("");
						}
						...
						if (flag) {
							String content = decodeData(
									...
									globalInfo.getPassWord(),
									globalInfo.getUserName()
											+ ((data.getQyZsbh() == null || data
													.getQyZsbh().equals("")) == true ? "1"
													: data.getQyZsbh()));
	...
	public static String decodeData(String org, String zipCode,
			String encryptCode, String password, String dsptbm)
			throws Exception {
		byte[] temp_coutnent;
		// 修改String串为空的判断
		if (StringUtils.isNotEmpty(org)) {
			...
			if ("1".equals(encryptCode)) {
				temp_coutnent = TripleDESUtil.decryptMode(password.substring(
						StaticConstant.SJMYBEFORELENG, password.length()),
						temp_coutnent);
			} else if ("2".equals(encryptCode)) {
				if (StaticConstant.CA_STATE.equals("1")) {// 税局端使用
					byte[] qyDer = (byte[]) StaticConstant.caPublicKeyMap
							.get(dsptbm);
					if (qyDer != null) {
					...
					String zspassword = (String) StaticConstant.caPublicKeypasswordMap
							.get(dsptbm);
					String filePath = (String) StaticConstant.caPublicKeyMap
							.get(dsptbm);
					Properties properties = new Properties();
					properties.load(StaticConstant.class
							.getResourceAsStream("/protocol.properties"));
					String derPath = URLDecoder.decode(StaticConstant.class
							.getResource("/ca").getPath())
							+ properties.getProperty(dsptbm.substring(0, 8)
									+ "SJ.Der");
					QYCaCryptUtil qyutil = new QYCaCryptUtil(
							FileUtils.readFileToByteArray(new File(derPath)),
							zspassword, filePath);// 动态读取税局端的公钥
					temp_coutnent = qyutil.decrypt(temp_coutnent);// 解密
				}
			}

			/**
			 * author: peterli modify date: 2013-11-26 21:44:01
			 */
			String content = new String(temp_coutnent, StaticConstant.CHARSET);
			// log.info("UTF-8编码下的content xml协议---------------------------" +
			// content);
	...
	public static String encodeData(String org, String zipCode,
			String encryptCode, String password, String dsptbm)
			throws UnsupportedEncodingException {
		byte[] temp_content = org == null ? "".getBytes() : org
				.getBytes(StaticConstant.CHARSET);
		try {
			if (StringUtils.isNotEmpty(org)) {
				if ("1".equals(encryptCode)) {
					temp_content = TripleDESUtil.encryptMode(password
							.substring(StaticConstant.SJMYBEFORELENG,
									password.length()), temp_content);
				} else if ("2".equals(encryptCode)) {
					if (StaticConstant.CA_STATE.equals("1")) {
						...
						QYCaCryptUtil qyutil = new QYCaCryptUtil(
								FileUtils
										.readFileToByteArray(new File(derPath)),
								zspassword, filePath);// 动态读取税局端的公钥
	...
	public static void getGlobalInfo(Map map, GlobalInfo globalInfo) {
		...
		globalInfo.setDataExchangeId((String) map.get(XmlPar.MAP_KEY_DSPTBM)
				...
				+ ProXml.getCurDate("yyyyMMdd") + getRdom(9));
	}
	...
	public static LogBean getLogBean(Map map, String type) {
		...
		try {
			...
			if (type.equals("response")) {
				logBean.setReturnMessage(ProXml.encode(returnStateInfoR
						.getReturnMessage()));
			}
			return logBean;
		} catch (Exception ce) {
	...
	// GlobalInfo log = ProXml.getGlobalInfo("123123","123123",
	// "123412312312312312");
	// ReturnStateInfo rs = ProXml.getReturnStateInfo("0000", "aaaa");
	// ByteArrayOutputStream out = new ByteArrayOutputStream();
	// out.write("<ROOT>2134234123412341234</ROOT>".getBytes());
	// Data data = ProXml.getData(out);
	// data.setEncryptCode("0");
	// data.setQyZsbh("qyzsbh");
	// data.setSjZsbh("sjzsbh");
	// out.close();
	// String xml = ProXml.getXml(log, rs, data);
	// System.out.println(xml);
	// FileUtils.writeByteArrayToFile(new File("c:\\abc.xml"),
	// xml.getBytes("UTF-8"));
	// String xml = new String(FileUtils.readFileToByteArray(new
	// File("c:\\abc.xml")),"UTF-8");
	// Map map = ProXml.getInterface(xml);
