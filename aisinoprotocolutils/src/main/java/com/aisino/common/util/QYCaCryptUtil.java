...
//	private javasafeengine jSE = null;
	...
	public QYCaCryptUtil(byte[] corpCert, String password, String filePath) {

		this.corpCert = corpCert;
		// this.sStoreName = StaticConstant.QY_StoreName; //密钥库名称
		this.sStoreName = filePath; // 密钥库名称
		this.sPin = password; // 密钥库私钥密码
...
//		sAlias = (String) jSE.getAliasEnum(kStore).nextElement();
	...
	/**
	 * 解密 cryptograph信封数据 signData签名数据
	 */
...
//		signedData = jSE.sign(cleartext, oPrivateKey, "SHA1/RSA", "BC");
	 ...
	 * KeyStoreException { String cleartext="航天信息"; String machineNo =
	 * "0000016500313026"; QYCaCryptUtil qyutil = new
	 * QYCaCryptUtil(FileUtils.readFileToByteArray(new
	 * File(StaticConstant.CA_TEST_PATH+"\\CorpCA\\UserCert.der"))); byte[]
	 * res=qyutil.encrypt(cleartext.getBytes());
	 * System.out.println("encrypt success:" +new
	 * String(qyutil.encrypt(cleartext.getBytes()))); SJCaCryptUtil sjutil = new
	 * SJCaCryptUtil(FileUtils.readFileToByteArray(new
	 * File(StaticConstant.CA_TEST_PATH+"\\qyCA\\UserCert.der"))); String strs[]
	 * = {"",""}; strs = new String(res).split(",");
	 * System.out.println("decrypt success: " + new
	 * String(sjutil.decrypt(res))); }
	 */
}
