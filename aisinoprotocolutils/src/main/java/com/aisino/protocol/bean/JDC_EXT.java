package com.aisino.protocol.bean;

public class JDC_EXT {
	private String CLLX;
	private String CPXH;
	private String CD;
	private String HGZH;
	private String JKZMSH;
	private String SJDH;
	private String FDJH;
	private String CLSBH;
	private String ZZSSE;
	private String DW;
	private String XCRS;
	// 20110713 新增
	private String FKFYB;
	private String ZJLX;
	private String SCQYMC;
	private String CLLXDL;

	public String getCLLX() {
		return CLLX;
	}

	public void setCLLX(String cLLX) {
		CLLX = cLLX;
	}

	public String getCPXH() {
		return CPXH;
	}

	public void setCPXH(String cPXH) {
		CPXH = cPXH;
	}

	public String getCD() {
		return CD;
	}

	public void setCD(String cD) {
		CD = cD;
	}

	public String getHGZH() {
		return HGZH;
	}

	public void setHGZH(String hGZH) {
		HGZH = hGZH;
	}

	public String getJKZMSH() {
		return JKZMSH;
	}

	public void setJKZMSH(String jKZMSH) {
		JKZMSH = jKZMSH;
	}

	public String getSJDH() {
		return SJDH;
	}

	public void setSJDH(String sJDH) {
		SJDH = sJDH;
	}

	public String getFDJH() {
		return FDJH;
	}

	public void setFDJH(String fDJH) {
		FDJH = fDJH;
	}

	public String getCLSBH() {
		return CLSBH;
	}

	public void setCLSBH(String cLSBH) {
		CLSBH = cLSBH;
	}

	public String getZZSSE() {
		return ZZSSE;
	}

	public void setZZSSE(String zZSSE) {
		ZZSSE = zZSSE;
	}

	public String getDW() {
		return DW;
	}

	public void setDW(String dW) {
		DW = dW;
	}

	public String getXCRS() {
		return XCRS;
	}

	public void setXCRS(String xCRS) {
		XCRS = xCRS;
	}

	public String getFKFYB() {
		return FKFYB;
	}

	public void setFKFYB(String fKFYB) {
		FKFYB = fKFYB;
	}

	public String getZJLX() {
		return ZJLX;
	}

	public void setZJLX(String zJLX) {
		ZJLX = zJLX;
	}

	public String getSCQYMC() {
		return SCQYMC;
	}

	public void setSCQYMC(String sCQYMC) {
		SCQYMC = sCQYMC;
	}

	public String getCLLXDL() {
		return CLLXDL;
	}

	public void setCLLXDL(String cLLXDL) {
		CLLXDL = cLLXDL;
	}
}