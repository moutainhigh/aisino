...
public class WebServiceCxfClient implements IWebServiceClient {

    private static final Logger LOGGER = LoggerFactory.getLogger(WebServiceCxfClient.class);
    
    public String clientData(String reqMethod, String reqXml, String url) {
        LOGGER.warn("创建factory开始----------------");
        String responeXml = "";
        JaxWsProxyFactoryBean factory = new JaxWsProxyFactoryBean();
        factory.setServiceClass(ReceiveShangHai.class);
        factory.setAddress(url);
        ReceiveShangHai u = (ReceiveShangHai) factory.create();
        LOGGER.warn("创建factory结束----------------");
        LOGGER.warn("发送报文开始----------------");
        responeXml = u.receive(reqXml);
        LOGGER.warn("发送报文结束----------------");
        if (responeXml == null) {
            return "";
        }
