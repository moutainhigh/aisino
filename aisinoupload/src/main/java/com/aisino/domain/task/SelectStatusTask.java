...
@Service
public final class SelectStatusTask {

    private final static org.slf4j.Logger LOGGER = LoggerFactory.getLogger(SelectStatusTask.class);
    ...
    /**
     * 查询发票状态为0的定时器 yaoxuguang
     */
    public void selectStatusIsZero() {
        DateTime begin1 = now();
        try {
            // 获取电商平台信息
            DateTime begin2 = now();
            ...
            for (Dsptxxkt dsptxx : dsptList) { /* 按电商分别扫描 */
                ...
                for (int i = 1; i <= page; i++) { // 按照页数循环
                    ...
                    map.put("count", size);
                    DateTime begin3 = now();
                    List<FPKJXX_FPTXX> fpkjList = (List<FPKJXX_FPTXX>) baseService.queryForList("selectStatusIsZero", map);
                    Long time3 = new Duration(begin3, now()).getMillis();
                    LOGGER.warn("SelectStatusTask定时器,获取数据库数据queryInvUploadForPlatform,耗时:{}", time3);
                    ...
                    if (fpkjList != null && fpkjList.size() > 0) {
                        for (int j = 0; j < fpkjList.size(); j++) {
                            ...
                            LOGGER.warn("SelectStatusTask定时器,放入上传51发票mq中,耗时:{}", time4);
                        }
                    } else {
                        bl = true;
                    }
                    Long time5 = new Duration(begin5, now()).getMillis();
                    LOGGER.warn("SelectStatusTask定时器,for循环单条,耗时:{}", time5);
                    // 如果没有数据跳出循环
                    if (bl) {
                        break;
                    }
        ...
        LOGGER.warn("SelectStatusTask定时器,selectStatusIsZero定时器结束,耗时:{}", time1);
    }
}
