 ...
 */
@Service
public final class InvoiceSend51PlatformHandler {

    private final static Logger LOGGER = LoggerFactory.getLogger(InvoiceSend51PlatformHandler.class);
    ...
    public void handleMessage(final SendPlatformQyeueEntity sendPlatformQyeueEntity) {
        ...
        LOGGER.warn("调用单张发票上传51服务sendPlatformServer方法用时{}毫秒", millSeconds);
    }

    private void sendPlatformServer(final SendPlatformQyeueEntity sendPlatformQyeueEntity) {
        LOGGER.warn("发票单张上传服务平台(51发票)任务开始...");
        ...
        try {
            ...
            LOGGER.warn("上传51发票,查询发票状态用时{}毫秒", millSeconds2);
            // 如果发票上传状态等于1,跳出本次业务
            if (fpsczt.equals(Constants.INV_51_SUCCESS)) {
                LOGGER.error("ID为{}的发票状态为{}，未执行发票上传51", invoiceId, fpsczt);
                return;
            }
            // 查询要上传51的发票
            ...
            if (!isNullOrEmpty(fpkj.getFP_DM()) && !isNullOrEmpty(fpkj.getFP_HM())) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("查询到未上传服务平台(51发票)的发票代码:{}, 纳税人识别号:{} 开始上传...", fpkj.getFP_DM(), fpkj.getNSRSBH());
                }
                ...
                LOGGER.warn("上传51发票,获取51平台路由url用时{}毫秒", millSeconds5);
                if (LOGGER.isDebugEnabled()) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("上传51发票,Map Param:{}", map.toString());
                }
                ...
                // 目前传递的都是单张的发票数据.
                // 调用上传51发票的接口,处理数据
                  boolean blresult = businessDataService.uploadInv(map, newArrayList(fpkj), route,2);
                ...
                LOGGER.warn("(上传51发票,uploadInv方法)发票{}上传51平台,总共耗时{}...", invoiceId, millSeconds1);
                ...
                if (blresult) {
                    ...
                    if ("Y".equals(pdfBlString)) {
                        ...
                        try {
                            ...
                            LOGGER.warn("(上传51发票,删除pdf,判断是否是文件并且文件存在)总共耗时{}...", millSeconds7);
                            DateTime begin8 = now();
                            ...
                            LOGGER.warn("(上传51发票,删除pdf,扫描空文件夹并删除)总共耗时{}...", millSeconds8);

                            Long millSeconds6 = new Duration(begin6, now()).getMillis();
                            LOGGER.warn("(上传51发票,删除pdf)总共耗时{}...", millSeconds6);
                        } catch (Exception e) {
                    ...
                    LOGGER.warn("(上传51发票)更新发票状态为失败状态(2)用时{}毫秒", millSeconds6);
                }
            } else {
                LOGGER.warn("发票单张上传服务平台(51发票), 没有获得需要上传的发票数据.");
            }

        } catch (SQLException e) {
            LOGGER.error("发票上传51业务sql异常", e);
        } catch (Exception e) {
            LOGGER.error("发票上传51业务异常", e);
        }
    }
    ...
    public static boolean deleteDirectory(String sPath) {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            LOGGER.warn("(上传51发票)删除单张PDF用时{}毫秒", millSeconds);
            if (!flag)
                continue;
        }
        if (!flag)
