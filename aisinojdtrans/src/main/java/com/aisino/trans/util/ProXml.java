...
public class ProXml {
	...
	public static String encode(String res){
		...
		return new String(base.encode(res.getBytes(SystemConfig.charSet)));
		}catch (Exception e) {
			log.error("未知：" + e);
			return "";
		}
	...
	public static String decode(String str) throws UnsupportedEncodingException {
		return new String(new Base64().decode(str.getBytes()),SystemConfig.charSet);
	}
	...
	public static String getTransXml(GlobalInfo globalInfo,ReturnStateInfo returnStateInfo, Data data){
		...
		taxpayerId.setText(globalInfo.getTaxpayerId());
		...
		String content=encodeData(data.getContent(), iszip, data.getEncryptCode(), globalInfo.getPassWord(),globalInfo.getTaxpayerId());
		...
		resData = byteRsp.toString(SystemConfig.charSet);
	} catch (IOException e) {
		log.error("未知：" + e);
	}finally{
		if(byteRsp != null){
			...
			if (node != null && node.size() > 0) {
				for (int i = 0; i < node.size(); i++) {
					...
					if (e1.getName().equals(XmlPar.GLOBALINFO)) {
						...
						globalInfo.setTaxpayerId(e1.getChild(XmlPar.TAXPAYERID)==null||e1.getChild(XmlPar.TAXPAYERID).getText()==null?"":e1.getChild(XmlPar.TAXPAYERID).getText());
		...
		try {
			...
			if ( StringUtils.isNotEmpty(org)) {
				...
				if("1".equals(encryptCode)){
					temp_coutnent = TripleDESUtil.decryptMode(password.substring(SystemConfig.SJMYBEFORELENG, password.length()), temp_coutnent);
				}else if("2".equals(encryptCode)){
					//CA解密
					 PKCS7 pkcs = new PKCS7();
					 temp_coutnent = pkcs.pkcs7Decrypt(temp_coutnent);
				}
				return new String(temp_coutnent,SystemConfig.charSet);
    ...
    public static String encodeData(String org, String zipCode, String encryptCode, String password,String nsrsbh) throws IOException {
        byte[] temp_content = org == null ? "".getBytes() : org.getBytes(SystemConfig.charSet);
//        try {
            if (StringUtils.isNotEmpty(org)) {
                if ("1".equals(encryptCode)) {
                    temp_content = TripleDESUtil.encryptMode(password.substring(SystemConfig.SJMYBEFORELENG, password.length()), temp_content);
                } else if ("2".equals(encryptCode)) {
                	byte[] publicPFXBytes = FileUtils.readFileToByteArray(new File(SystemConfig.NSR_CA_PUBLIC_KEY.replace("~", nsrsbh)));
                	PKCS7 pkcs = new PKCS7();
                	temp_content = pkcs.pkcs7Encrypt(org, publicPFXBytes);
                }
                if ("1".equals(zipCode)) {
                    temp_content = GZipUtils.compress(temp_content);
                }
//                return new String(encode(temp_content), SystemConfig.charSet);
...
//        }
	...
	public static Data getData(ByteArrayOutputStream out) {
		Data data = new Data();
		try {
			data.setEncryptCode(SystemConfig.PROTOCOL_ENCRYPTCODE);
			data.setCodeType(SystemConfig.PROTOCOL_CODETYPE);
			String temp_str = new String(out.toByteArray(),SystemConfig.charSet);
