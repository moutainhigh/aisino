package com.aisino.protocol.bean;

public class ESC_EXT
{
	private String CPZH;
	private String DJZH;
	private String CLLX;
	private String CLSBH;
	private String CPXH;
	private String CGSMC;
	private String JYDWMC;
	private String JYDWDZ;
	private String JYDWSBH;
	private String JYDWKHYH;
	private String JYDWDH;
	private String GSBMMC;
	
	private String SCMC;
	private String SCSBH;
	private String SCDZ;
	private String SCKHYH;
	private String SCDH;
	private String MFMC;
	private String MFDM;
	private String MFDZ;
	private String MFDH;
	
	public String getCPZH() {
		return CPZH;
	}
	public void setCPZH(String cPZH) {
		CPZH = cPZH;
	}
	public String getDJZH() {
		return DJZH;
	}
	public void setDJZH(String dJZH) {
		DJZH = dJZH;
	}
	public String getCLLX() {
		return CLLX;
	}
	public void setCLLX(String cLLX) {
		CLLX = cLLX;
	}
	public String getCLSBH() {
		return CLSBH;
	}
	public void setCLSBH(String cLSBH) {
		CLSBH = cLSBH;
	}
	public String getCPXH() {
		return CPXH;
	}
	public void setCPXH(String cPXH) {
		CPXH = cPXH;
	}
	public String getCGSMC() {
		return CGSMC;
	}
	public void setCGSMC(String cGSMC) {
		CGSMC = cGSMC;
	}
	public String getJYDWMC() {
		return JYDWMC;
	}
	public void setJYDWMC(String jYDWMC) {
		JYDWMC = jYDWMC;
	}
	public String getJYDWDZ() {
		return JYDWDZ;
	}
	public void setJYDWDZ(String jYDWDZ) {
		JYDWDZ = jYDWDZ;
	}
	public String getJYDWSBH() {
		return JYDWSBH;
	}
	public void setJYDWSBH(String jYDWSBH) {
		JYDWSBH = jYDWSBH;
	}
	public String getJYDWKHYH() {
		return JYDWKHYH;
	}
	public void setJYDWKHYH(String jYDWKHYH) {
		JYDWKHYH = jYDWKHYH;
	}
	public String getJYDWDH() {
		return JYDWDH;
	}
	public void setJYDWDH(String jYDWDH) {
		JYDWDH = jYDWDH;
	}
	public String getGSBMMC() {
		return GSBMMC;
	}
	public void setGSBMMC(String gSBMMC) {
		GSBMMC = gSBMMC;
	}
	public String getSCMC() {
		return SCMC;
	}
	public void setSCMC(String sCMC) {
		SCMC = sCMC;
	}
	public String getSCSBH() {
		return SCSBH;
	}
	public void setSCSBH(String sCSBH) {
		SCSBH = sCSBH;
	}
	public String getSCDZ() {
		return SCDZ;
	}
	public void setSCDZ(String sCDZ) {
		SCDZ = sCDZ;
	}
	public String getSCKHYH() {
		return SCKHYH;
	}
	public void setSCKHYH(String sCKHYH) {
		SCKHYH = sCKHYH;
	}
	public String getSCDH() {
		return SCDH;
	}
	public void setSCDH(String sCDH) {
		SCDH = sCDH;
	}
	public String getMFMC() {
		return MFMC;
	}
	public void setMFMC(String mFMC) {
		MFMC = mFMC;
	}
	public String getMFDM() {
		return MFDM;
	}
	public void setMFDM(String mFDM) {
		MFDM = mFDM;
	}
	public String getMFDZ() {
		return MFDZ;
	}
	public void setMFDZ(String mFDZ) {
		MFDZ = mFDZ;
	}
	public String getMFDH() {
		return MFDH;
	}
	public void setMFDH(String mFDH) {
		MFDH = mFDH;
	}
	

}
