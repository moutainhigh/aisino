package com.aisino.protocol.bean;

/**
 * <p>
 * [描述信息：发票开具生成后PDF文件下发协议]
 * </p>
 * 
 * @author wuyong@aisino.com
 * @version 1.0 Created on Nov 4, 2013 9:03:26 PM
 */
public class FPKJXX_FPJGXX {

	private String KPLSH;// 开票流水号
	private String FWM;// 防伪码
	private String EWM;// 二维码
	private String FPZL_DM;// FPZL_DM
	private String FP_DM;// 发票代码
	private String FP_HM;// 发票号码
	private String PDF_FILE;// 电子发票PDF
	private String DDH;// 发票订单号码
	private String FPQQLSH;// 发票请求唯一流水号

	public String getKPLSH() {
		return KPLSH;
	}

	public void setKPLSH(String kplsh) {
		KPLSH = kplsh;
	}

	public String getFWM() {
		return FWM;
	}

	public void setFWM(String fwm) {
		FWM = fwm;
	}

	public String getEWM() {
		return EWM;
	}

	public void setEWM(String ewm) {
		EWM = ewm;
	}

	public String getFPZL_DM() {
		return FPZL_DM;
	}

	public void setFPZL_DM(String fpzl_dm) {
		FPZL_DM = fpzl_dm;
	}

	public String getFP_DM() {
		return FP_DM;
	}

	public void setFP_DM(String fp_dm) {
		FP_DM = fp_dm;
	}

	public String getFP_HM() {
		return FP_HM;
	}

	public void setFP_HM(String fp_hm) {
		FP_HM = fp_hm;
	}

	public String getPDF_FILE() {
		return PDF_FILE;
	}

	public void setPDF_FILE(String pdf_file) {
		PDF_FILE = pdf_file;
	}

	public String getDDH() {
		return DDH;
	}

	public void setDDH(String ddh) {
		DDH = ddh;
	}

	public String getFPQQLSH() {
		return FPQQLSH;
	}

	public void setFPQQLSH(String fpqqlsh) {
		FPQQLSH = fpqqlsh;
	}
}
