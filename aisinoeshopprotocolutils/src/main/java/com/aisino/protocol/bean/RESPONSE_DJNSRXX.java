package com.aisino.protocol.bean;

import com.aisino.protocol.bean.*;

public class RESPONSE_DJNSRXX extends com.aisino.protocol.bean.RESPONSE_BEAN {
	private String NSRDZDAH;
	private String WSPZXH;
	private String NSRSBH;
	private String NSRBM;
	private String NSRMC;
	private String FDDBRMC;
	private String FRZJLX_DM;
	private String ZJHM;
	private String SCJYDZ;
	private String BSRMC;
	private String DHHM;
	private String LSGX_DM;
	private String HY_DM;
	private String DJZCLX_DM;
	private String HZDJRQ;
	private String QYKJZD_DM;
	private String SWDJBLX_DM;
	private String NSRZT_DM;
	private String ZGSWRY_DM;
	private String JDXZ_DM;
	private String ZJG_BZ;
	private String NSR_SWJG_DM;
	private String SWJG_DM;
	private String LRR_DM;
	private String LRRQ;
	private String XGR_DM;
	private String XGRQ;
	private String GSZGSWJG_J_DM;
	private String DWXZ_DM;
	private String GGHBZ;
	private String ZY;
	private String JY;
	private String JYFW;
	private String KPMS;
	private String NSRLX_DM;
	private String DJSJ;
	private String KPGLY_MC;
	private String PWD;
	private String CZY_DM;
	private String KPD_SL;
	private String NSRZT;
	private String ZDLX_KP_TS;
	private String ZD_KP_ZS;
	private String LJ_KP_JE;
	private String CB_ZQ;
	private String ZD_LX;
	private String HY_DM_WP;
	private String ESC_LX;
	private String KPFS;
	private String RETURNCODE;
    private String RETURNMESSAGE;
    public String getRETURNCODE() {
        return RETURNCODE;
    }
    public void setRETURNCODE(String returncode) {
        RETURNCODE = returncode;
    }
    public String getRETURNMESSAGE() {
        return RETURNMESSAGE;
    }
    public void setRETURNMESSAGE(String returnmessage) {
        RETURNMESSAGE = returnmessage;
    }
	public String getNSRDZDAH() {
		return NSRDZDAH;
	}
	public void setNSRDZDAH(String nSRDZDAH) {
		NSRDZDAH = nSRDZDAH;
	}
	public String getWSPZXH() {
		return WSPZXH;
	}
	public void setWSPZXH(String wSPZXH) {
		WSPZXH = wSPZXH;
	}
	public String getNSRSBH() {
		return NSRSBH;
	}
	public void setNSRSBH(String nSRSBH) {
		NSRSBH = nSRSBH;
	}
	public String getNSRBM() {
		return NSRBM;
	}
	public void setNSRBM(String nSRBM) {
		NSRBM = nSRBM;
	}
	public String getNSRMC() {
		return NSRMC;
	}
	public void setNSRMC(String nSRMC) {
		NSRMC = nSRMC;
	}
	public String getFDDBRMC() {
		return FDDBRMC;
	}
	public void setFDDBRMC(String fDDBRMC) {
		FDDBRMC = fDDBRMC;
	}
	public String getFRZJLX_DM() {
		return FRZJLX_DM;
	}
	public void setFRZJLX_DM(String fRZJLX_DM) {
		FRZJLX_DM = fRZJLX_DM;
	}
	public String getZJHM() {
		return ZJHM;
	}
	public void setZJHM(String zJHM) {
		ZJHM = zJHM;
	}
	public String getSCJYDZ() {
		return SCJYDZ;
	}
	public void setSCJYDZ(String sCJYDZ) {
		SCJYDZ = sCJYDZ;
	}
	public String getBSRMC() {
		return BSRMC;
	}
	public void setBSRMC(String bSRMC) {
		BSRMC = bSRMC;
	}
	public String getDHHM() {
		return DHHM;
	}
	public void setDHHM(String dHHM) {
		DHHM = dHHM;
	}
	public String getLSGX_DM() {
		return LSGX_DM;
	}
	public void setLSGX_DM(String lSGX_DM) {
		LSGX_DM = lSGX_DM;
	}
	public String getHY_DM() {
		return HY_DM;
	}
	public void setHY_DM(String hY_DM) {
		HY_DM = hY_DM;
	}
	public String getDJZCLX_DM() {
		return DJZCLX_DM;
	}
	public void setDJZCLX_DM(String dJZCLX_DM) {
		DJZCLX_DM = dJZCLX_DM;
	}
	public String getHZDJRQ() {
		return HZDJRQ;
	}
	public void setHZDJRQ(String hZDJRQ) {
		HZDJRQ = hZDJRQ;
	}
	public String getQYKJZD_DM() {
		return QYKJZD_DM;
	}
	public void setQYKJZD_DM(String qYKJZD_DM) {
		QYKJZD_DM = qYKJZD_DM;
	}
	public String getSWDJBLX_DM() {
		return SWDJBLX_DM;
	}
	public void setSWDJBLX_DM(String sWDJBLX_DM) {
		SWDJBLX_DM = sWDJBLX_DM;
	}
	public String getNSRZT_DM() {
		return NSRZT_DM;
	}
	public void setNSRZT_DM(String nSRZT_DM) {
		NSRZT_DM = nSRZT_DM;
	}
	public String getZGSWRY_DM() {
		return ZGSWRY_DM;
	}
	public void setZGSWRY_DM(String zGSWRY_DM) {
		ZGSWRY_DM = zGSWRY_DM;
	}
	public String getJDXZ_DM() {
		return JDXZ_DM;
	}
	public void setJDXZ_DM(String jDXZ_DM) {
		JDXZ_DM = jDXZ_DM;
	}
	public String getZJG_BZ() {
		return ZJG_BZ;
	}
	public void setZJG_BZ(String zJG_BZ) {
		ZJG_BZ = zJG_BZ;
	}
	public String getNSR_SWJG_DM() {
		return NSR_SWJG_DM;
	}
	public void setNSR_SWJG_DM(String nSR_SWJG_DM) {
		NSR_SWJG_DM = nSR_SWJG_DM;
	}
	public String getSWJG_DM() {
		return SWJG_DM;
	}
	public void setSWJG_DM(String sWJG_DM) {
		SWJG_DM = sWJG_DM;
	}
	public String getLRR_DM() {
		return LRR_DM;
	}
	public void setLRR_DM(String lRR_DM) {
		LRR_DM = lRR_DM;
	}
	public String getLRRQ() {
		return LRRQ;
	}
	public void setLRRQ(String lRRQ) {
		LRRQ = lRRQ;
	}
	public String getXGR_DM() {
		return XGR_DM;
	}
	public void setXGR_DM(String xGR_DM) {
		XGR_DM = xGR_DM;
	}
	public String getXGRQ() {
		return XGRQ;
	}
	public void setXGRQ(String xGRQ) {
		XGRQ = xGRQ;
	}
	public String getGSZGSWJG_J_DM() {
		return GSZGSWJG_J_DM;
	}
	public void setGSZGSWJG_J_DM(String gSZGSWJG_J_DM) {
		GSZGSWJG_J_DM = gSZGSWJG_J_DM;
	}
	public String getDWXZ_DM() {
		return DWXZ_DM;
	}
	public void setDWXZ_DM(String dWXZ_DM) {
		DWXZ_DM = dWXZ_DM;
	}
	public String getGGHBZ() {
		return GGHBZ;
	}
	public void setGGHBZ(String gGHBZ) {
		GGHBZ = gGHBZ;
	}
	public String getZY() {
		return ZY;
	}
	public void setZY(String zY) {
		ZY = zY;
	}
	public String getJY() {
		return JY;
	}
	public void setJY(String jY) {
		JY = jY;
	}
	public String getJYFW() {
		return JYFW;
	}
	public void setJYFW(String jYFW) {
		JYFW = jYFW;
	}
	public String getKPMS() {
		return KPMS;
	}
	public void setKPMS(String kPMS) {
		KPMS = kPMS;
	}
	public String getNSRLX_DM() {
		return NSRLX_DM;
	}
	public void setNSRLX_DM(String nSRLX_DM) {
		NSRLX_DM = nSRLX_DM;
	}
	public String getDJSJ() {
		return DJSJ;
	}
	public void setDJSJ(String dJSJ) {
		DJSJ = dJSJ;
	}
	public String getKPGLY_MC() {
		return KPGLY_MC;
	}
	public void setKPGLY_MC(String kPGLY_MC) {
		KPGLY_MC = kPGLY_MC;
	}
	public String getPWD() {
		return PWD;
	}
	public void setPWD(String pWD) {
		PWD = pWD;
	}
	public String getCZY_DM() {
		return CZY_DM;
	}
	public void setCZY_DM(String cZY_DM) {
		CZY_DM = cZY_DM;
	}
	public String getKPD_SL() {
		return KPD_SL;
	}
	public void setKPD_SL(String kPD_SL) {
		KPD_SL = kPD_SL;
	}
	public String getNSRZT() {
		return NSRZT;
	}
	public void setNSRZT(String nSRZT) {
		NSRZT = nSRZT;
	}
	public String getZDLX_KP_TS() {
		return ZDLX_KP_TS;
	}
	public void setZDLX_KP_TS(String zDLX_KP_TS) {
		ZDLX_KP_TS = zDLX_KP_TS;
	}
	public String getZD_KP_ZS() {
		return ZD_KP_ZS;
	}
	public void setZD_KP_ZS(String zD_KP_ZS) {
		ZD_KP_ZS = zD_KP_ZS;
	}
	public String getLJ_KP_JE() {
		return LJ_KP_JE;
	}
	public void setLJ_KP_JE(String lJ_KP_JE) {
		LJ_KP_JE = lJ_KP_JE;
	}
	public String getCB_ZQ() {
		return CB_ZQ;
	}
	public void setCB_ZQ(String cB_ZQ) {
		CB_ZQ = cB_ZQ;
	}
	public String getZD_LX() {
		return ZD_LX;
	}
	public void setZD_LX(String zD_LX) {
		ZD_LX = zD_LX;
	}
	public String getHY_DM_WP() {
		return HY_DM_WP;
	}
	public void setHY_DM_WP(String hY_DM_WP) {
		HY_DM_WP = hY_DM_WP;
	}
	public String getESC_LX() {
		return ESC_LX;
	}
	public void setESC_LX(String eSC_LX) {
		ESC_LX = eSC_LX;
	}
	public String getKPFS() {
		return KPFS;
	}
	public void setKPFS(String kPFS) {
		KPFS = kPFS;
	}
	
}
