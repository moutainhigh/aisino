package com.aisino.protocol.bean;

public class CX_RESULT {
	private String FP_CY_ID;
	private String FP_KJID;
	private String CX_RESULT_DM;
	private String CX_RESULT_DETAIL;
	private String FPDM;
	private String FPHM;
	private String FWM;
	private String KPJE;
	private String KPFSBH;
	private String KPFMC;
	private String KPRQ;
	private String KHMC;
	private String KHSBM;
	private String KHDH;
	private String KHDZ;
	private String WLDDH;
	private String ZDY1;
	private String ZDY2;
	private String ZDY3;
	private String ZDY4;
	private String ZDY5;
	
	
	public String getFP_CY_ID() {
		return FP_CY_ID;
	}
	public void setFP_CY_ID(String fP_CY_ID) {
		FP_CY_ID = fP_CY_ID;
	}
	
	public String getFP_KJID() {
		return FP_KJID;
	}
	public void setFP_KJID(String fP_KJID) {
		FP_KJID = fP_KJID;
	}
	public String getCX_RESULT_DM() {
		return CX_RESULT_DM;
	}
	public void setCX_RESULT_DM(String cX_RESULT_DM) {
		CX_RESULT_DM = cX_RESULT_DM;
	}
	public String getCX_RESULT_DETAIL() {
		return CX_RESULT_DETAIL;
	}
	public void setCX_RESULT_DETAIL(String cX_RESULT_DETAIL) {
		CX_RESULT_DETAIL = cX_RESULT_DETAIL;
	}
	public String getFPDM() {
		return FPDM;
	}
	public void setFPDM(String fPDM) {
		FPDM = fPDM;
	}
	public String getFPHM() {
		return FPHM;
	}
	public void setFPHM(String fPHM) {
		FPHM = fPHM;
	}
	public String getFWM() {
		return FWM;
	}
	public void setFWM(String fWM) {
		FWM = fWM;
	}
	public String getKPJE() {
		return KPJE;
	}
	public void setKPJE(String kPJE) {
		KPJE = kPJE;
	}
	public String getKPFSBH() {
		return KPFSBH;
	}
	public void setKPFSBH(String kPFSBH) {
		KPFSBH = kPFSBH;
	}
	public String getKPFMC() {
		return KPFMC;
	}
	public void setKPFMC(String kPFMC) {
		KPFMC = kPFMC;
	}
	public String getKPRQ() {
		return KPRQ;
	}
	public void setKPRQ(String kPRQ) {
		KPRQ = kPRQ;
	}
	public String getKHMC() {
		return KHMC;
	}
	public void setKHMC(String kHMC) {
		KHMC = kHMC;
	}
	public String getKHSBM() {
		return KHSBM;
	}
	public void setKHSBM(String kHSBM) {
		KHSBM = kHSBM;
	}
	public String getKHDH() {
		return KHDH;
	}
	public void setKHDH(String kHDH) {
		KHDH = kHDH;
	}
	public String getKHDZ() {
		return KHDZ;
	}
	public void setKHDZ(String kHDZ) {
		KHDZ = kHDZ;
	}
	public String getWLDDH() {
		return WLDDH;
	}
	public void setWLDDH(String wLDDH) {
		WLDDH = wLDDH;
	}
	public String getZDY1() {
		return ZDY1;
	}
	public void setZDY1(String zDY1) {
		ZDY1 = zDY1;
	}
	public String getZDY2() {
		return ZDY2;
	}
	public void setZDY2(String zDY2) {
		ZDY2 = zDY2;
	}
	public String getZDY3() {
		return ZDY3;
	}
	public void setZDY3(String zDY3) {
		ZDY3 = zDY3;
	}
	public String getZDY4() {
		return ZDY4;
	}
	public void setZDY4(String zDY4) {
		ZDY4 = zDY4;
	}
	public String getZDY5() {
		return ZDY5;
	}
	public void setZDY5(String zDY5) {
		ZDY5 = zDY5;
	}
}
