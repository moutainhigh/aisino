package com.aisino.protocol.bean;

public class JZY_EXT {
	private String SPF_SBH;
	private String FPDM;
	private String FPHM;
	private String KPXM_XH;
	private String ZBRBZ;
	private String FBRBZ;
	private String GCXMMC;
	private String GCXMBH;
	private String JSXM;
	private String JE;
	private String WSPZ_HM;
	private String WSPZ_ZL;
	private String BZ;
	public String getSPF_SBH() {
		return SPF_SBH;
	}
	public void setSPF_SBH(String sPF_SBH) {
		SPF_SBH = sPF_SBH;
	}
	public String getFPDM() {
		return FPDM;
	}
	public void setFPDM(String fPDM) {
		FPDM = fPDM;
	}
	public String getFPHM() {
		return FPHM;
	}
	public void setFPHM(String fPHM) {
		FPHM = fPHM;
	}
	public String getKPXM_XH() {
		return KPXM_XH;
	}
	public void setKPXM_XH(String kPXM_XH) {
		KPXM_XH = kPXM_XH;
	}
	public String getZBRBZ() {
		return ZBRBZ;
	}
	public void setZBRBZ(String zBRBZ) {
		ZBRBZ = zBRBZ;
	}
	public String getFBRBZ() {
		return FBRBZ;
	}
	public void setFBRBZ(String fBRBZ) {
		FBRBZ = fBRBZ;
	}
	public String getGCXMMC() {
		return GCXMMC;
	}
	public void setGCXMMC(String gCXMMC) {
		GCXMMC = gCXMMC;
	}
	public String getGCXMBH() {
		return GCXMBH;
	}
	public void setGCXMBH(String gCXMBH) {
		GCXMBH = gCXMBH;
	}
	public String getJSXM() {
		return JSXM;
	}
	public void setJSXM(String jSXM) {
		JSXM = jSXM;
	}
	public String getJE() {
		return JE;
	}
	public void setJE(String jE) {
		JE = jE;
	}
	public String getWSPZ_HM() {
		return WSPZ_HM;
	}
	public void setWSPZ_HM(String wSPZ_HM) {
		WSPZ_HM = wSPZ_HM;
	}
	public String getWSPZ_ZL() {
		return WSPZ_ZL;
	}
	public void setWSPZ_ZL(String wSPZ_ZL) {
		WSPZ_ZL = wSPZ_ZL;
	}
	public String getBZ() {
		return BZ;
	}
	public void setBZ(String bZ) {
		BZ = bZ;
	}
	
}
